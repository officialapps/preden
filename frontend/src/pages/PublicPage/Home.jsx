...
import { Search, ChevronDown, MoreHorizontal, AlertCircle, Loader2 } from "lucide-react"
...
const Home = () => {
  ...
  const { eventsData, categoryList, isLoading, errorMessage, refetch, getFilteredEvents } = useEventsData()
  ...
  const getTokenIcon = (tokenAddress) => {
    ...
    if (normalizedAddress === "0x0" || normalizedAddress === "0x0000000000000000000000000000000000000000") {
      return USDT
    }
          ...
          <div className="pb-28">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-lg md:text-xl lg:text-2xl font-bold text-cyan-400">PREDICT</span>
                <img
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  >
                    <MoreHorizontal className="w-5 h-5 md:w-6 md:h-6 text-gray-400" />
                  </div>
                  {showDropdown && (
    ...
    <div className="relative p-4 md:p-6 lg:p-8 min-h-screen">
      ...
      <div className="relative z-10 max-w-screen-xl mx-auto">
        <div className="pb-28">
          ...
          {errorMessage && (
            <div className="bg-red-500 bg-opacity-20 border border-red-500 text-white p-3 rounded-lg mb-4 flex items-center">
          ...
          <div className="relative mt-5 mb-6">
            ...
            >
              <div className="flex items-center bg-[#09113B] rounded-full p-3 md:p-4 cursor-pointer">
                <Search className="w-5 h-5 md:w-6 md:h-6 text-gray-400 mr-2" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => {
                  ...
                  placeholder="Search"
                  className="bg-transparent outline-none flex-1 text-white placeholder-gray-400 text-sm md:text-base"
                />
                <div className="relative">
                  <ChevronDown
                      ...
                      {categoryList.map((category) => (
                        <div
                          ...
                          className={`px-4 py-2 md:py-3 cursor-pointer hover:bg-[#252B4F] transition-colors ${
                            selectedCategory === category.id ? "bg-[#252B4F] text-cyan-400" : "text-white"
                          }`}
          ...
          <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3">
            {visibleEvents.length > 0 ? (
                  ...
                  <div className="bg-[#09113B] rounded-2xl p-4 h-full flex flex-col min-h-[155px]">
                    <div className="flex items-start gap-3 mb-1">
                      ...
                      <div className="flex-1 min-h-[2.5rem]">
                        <h3 className="font-semibold text-white text-sm md:text-base leading-tight line-clamp-2">{event.question}</h3>
                      </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {event.options.A}
                        <img src={Up || "/placeholder.svg"} alt="Option A icon" className="w-3 h-3" />
                      </button>
                      ...
                      >
                        {event.options.B}
                        <img src={Down || "/placeholder.svg"} alt="Option B icon" className="w-3 h-3" />
                      </button>
                    </div>

                    <div className="flex items-center justify-between text-xs text-gray-400 mt-auto">
                      <div className="flex items-center gap-1.5">
                        <img src={getTokenIcon(event.tokenAddress)} alt="token icon" className="w-3 h-3" />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00FFFF] to-[#27FE60]">
                          vol. {formatVolume(event.yes_votes + event.no_votes, event.tokenAddress)}
                        </span>
                      </div>
                      <div className="flex items-center gap-1.5">
                      ...
                      <div className="flex items-center justify-center">
                        ...
                        >
                          <button className="flex items-center justify-center">
                            <img src={Share || "/placeholder.svg"} alt="share icon" className="w-3 h-3" />
                          </button>
                        </RWebShare>
                      </div>
                    </div>
              ...
              <div className="col-span-full text-center py-8">
                <p className="text-white text-lg">No active events found</p>
              </div>
            )}
          </div>
          ...
          {visibleItems < activeEvents.length && (
            <div className="flex justify-center mt-6">
              ...
              >
                See More ({activeEvents.length - visibleItems} more active events)
              </button>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-gray-800 bg-opacity-75 flex justify-center items-center p-4">
          <div className="bg-[#1A1F3F] p-6 rounded-lg w-full max-w-md md:max-w-lg lg:max-w-xl">
            <div className="flex justify-between items-center">
              <h2 className="text-lg md:text-xl lg:text-2xl text-white font-bold">Event Info</h2>
              <button className="text-white text-xl md:text-2xl hover:text-cyan-400" onClick={closeModal}>
                &times;
              </button>
            </div>
            <div className="mt-4 text-white text-sm md:text-base">
              ...
              <p className="mb-2">
                <strong>Description:</strong> {selectedEvent.description || "No additional information available."}
              </p>
              <p className="mb-2">
                <strong>Category:</strong> {selectedEvent.category?.label || "Unknown"}
              </p>
              <p className="mb-2">
                <strong>End Date:</strong> {new Date(selectedEvent.end_time).toLocaleString()}
              </p>
              <p className="mb-2">
                <strong>Time Remaining:</strong> {formatTimeRemaining(selectedEvent.end_time)}
              </p>
              <p className="mb-2">
                ...
                <div className="flex gap-4 mt-2">
                  <div className="flex-1 bg-[#51cddd20] text-[#18DDF7] py-2 px-4 rounded-lg">
                    <div className="text-center">
                      <div className="font-semibold">{selectedEvent.options.A}</div>
                      <div className="text-xs">{formatVoteCount(selectedEvent.yes_votes, selectedEvent.tokenAddress)} {getTokenSymbol(selectedEvent.tokenAddress)}</div>
                    </div>
                  </div>
                  <div className="flex-1 bg-[#ff5a5a20] text-[#FF5A5A] py-2 px-4 rounded-lg">
                    <div className="text-center">
                      <div className="font-semibold">{selectedEvent.options.B}</div>
                      <div className="text-xs">{formatVoteCount(selectedEvent.no_votes, selectedEvent.tokenAddress)} {getTokenSymbol(selectedEvent.tokenAddress)}</div>
                    </div>
                  </div>
                </div>
                <div className="mt-2 text-center text-xs text-gray-400">
                  Total Volume: {formatVolume(selectedEvent.yes_votes + selectedEvent.no_votes, selectedEvent.tokenAddress)} {getTokenSymbol(selectedEvent.tokenAddress)}
                </div>
              </div>
            </div>
